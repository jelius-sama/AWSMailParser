...
build_cmd() {
  printf 'CGO_ENABLED=0 GOOS=%s GOARCH=%s go build -trimpath -buildvcs=false -o .%s/%s-%s-%s-%s ./cmd/' \
    "$1" "$2" "$BIN_DIR" "$TITLE" "$VERSION" "$1" "$2"
}
...
usage() {
    ...
    echo
    echo "Examples:"
    echo "  ./build.sh       -> builds .${BIN_DIR}/${TITLE}-${VERSION}"
    echo "  ./build.sh all   -> builds multiple binaries into ./bin/"
    exit 1
}

if [ $# -eq 0 ]; then
    build "host_default" CGO_ENABLED=0 go build -trimpath -buildvcs=false -o .${BIN_DIR}/${TITLE}-${VERSION} ./cmd/            
